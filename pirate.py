"""
ou must hurry to find Professor Boolean's secret lab. You fear it may be too late. 
...
def answer(numbers):
	start =0
	redirect = numbers[start]
	has_seen = {}
	current_count=0
	has_seen[redirect]= current_count
	current_count+=1
	break_count=0
	while True:
		redirect= numbers[redirect]
		if redirect >= len(numbers):
			return 0
		if redirect in has_seen.keys():
			break_count= has_seen[redirect]
			break;
		else:
			has_seen[redirect]=current_count
			current_count+=1
	return len(has_seen.keys()) -break_count

print(answer([1, 2,3,2]))